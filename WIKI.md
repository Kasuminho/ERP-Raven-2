...
- O job `quality` sobe PostgreSQL 16 descartavel `raven2_e2e`, aplica todas as migrations com `prisma migrate deploy` e executa `npm run test:e2e`. O E2E usa o Nest/HTTP real e prova evento-presenca-DKP, leilao-lock-review-entrega-recibo privado, request/interesse-decisao-entrega e recrutamento-conversao-onboarding; o runner recusa URLs que nao identifiquem banco isolado.
...
- A rotacao atual cobre embeds, DMs, partilha de diamantes, healthcheck, DKP-LOG, resumo semanal e punchlines do changelog, com bancos renovados em 2026-08-03 para evitar reciclar as frases das revisoes anteriores.
...
- O modo manutencao usa a regra de negocio `maintenanceMode` com `{ enabled, message }`. Quando ativo, um guard global bloqueia mutacoes sensiveis em leiloes, finalizacao/automacao, entregas/drops, ajustes DKP, progresso, requests, anuncios, interesses, eventos, casos privados, Codex, Daoshi, uploads e webhooks operacionais; leituras e health continuam liberados. `PATCH /business-rules/maintenanceMode` permanece liberado para Staff desativar o modo, e `GET /operations/maintenance` alimenta o banner da Web.
...
- Todos os controllers que recebem body de mutacao usam `whitelist`/`forbidNonWhitelisted` local e DTOs decorados. `auctions`, `dkp`, `drops`, `items`, `item-interests`, `operations` e `discord` foram migrados por contrato em 2026-07-21; IDs de autoria e o alvo de sync Discord sempre vêm da sessao autenticada. O pipe global permanece apenas com `transform` para nao reinterpretar DTOs legados sem uso em body; novas mutacoes devem nascer com pipe local estrito e teste de contrato.
...
- Na Web, `apps/web/src/hooks/use-guild-api.ts` e apenas um barrel temporario. Hooks ficam separados por dominio: `use-profile-api`, `use-dkp-api`, `use-staff-operations-api`, `use-auctions-api`, `use-items-api`, `use-requests-api`, `use-events-api`, `use-codex-api`, `use-daoshi-api`, `use-drops-api` e `use-diamond-sales-api`; telas novas devem importar direto do dominio.
...
- O cron `sendPlayerDailyReminders` roda todos os dias as 12:30 em `America/Sao_Paulo` e cobra cada player ativo por notificacao interna e DM bilingue consolidada. Os sinais diarios sao build ausente, role ausente, disponibilidade ausente, STATUS ausente/mais antigo que 21 dias e presenca abaixo de 50% nos eventos finalizados dos ultimos 15 dias posteriores ao ingresso do player; sem evento elegivel, nao ha sinal de presenca baixa. `CodexRequest` em `SENT` tambem entra ate confirmacao ou retry, alem da DM imediata ao envio pela Staff. `PlayerReminderDelivery` e `Notification.deduplicationKey` garantem idempotencia por player/dia e retry controlado de falha no Discord.
- RSVP generico usa `EventRsvp`, unico por evento/player, com `CONFIRMED`, `TENTATIVE` ou `DECLINED`, nota opcional e visibilidade `STAFF_ONLY` por padrao ou `PLAYER_PUBLIC`. Players usam `GET /events/commitments/me` e `PUT /events/:id/rsvp` na tela `/dashboard/attendance`; a resposta pode mudar ate o inicio. Staff usa `GET /events/:id/rsvp/staff` no evento selecionado para contagens e composicao confirmada por classe, role e camada. Notas publicas aparecem a outros players; notas privadas ficam somente para autor e Staff. RSVP nao cria presenca, nao distribui DKP e permanece independente por boss.
- Periodos de ausencia usam `PlayerAbsence`, com inicio/fim, motivo opcional e `reasonVisibility` Staff-only por padrao. O player lista/cria/edita/remove por `/events/absences/me` e gerencia na tela de Presenca. Eventos cujo `startsAt` cai no intervalo ficam `indisponiveis` sem exigir RSVP individual; o resumo Staff desconta esses players de confirmados/sem resposta, mostra `UNAVAILABLE_BY_ABSENCE`, identidade e motivo. Para outros players, a agenda expoe apenas a contagem agregada de indisponiveis e nome/motivo somente quando `PLAYER_PUBLIC`. O fluxo nao envia cobranca ou notificacao de ausencia.
- Recorrencia de eventos usa `EventSeries`: primeira ocorrencia, duracao, intervalo semanal, timezone IANA, horizonte, excecoes, pausa e targets de composicao. `materializeEventSeries` roda diariamente as 02:15 em `America/Sao_Paulo`, cria instancias idempotentes por `eventSeriesId/seriesOccurrence` e amplia o horizonte. Pausar cancela instancias futuras da serie com marcador reversivel; retomar restaura apenas datas que nao continuam como excecao. Endpoints Staff ficam em `/events/series`.
- `Event.compositionTargets` armazena minimos por role e/ou classe. O resumo RSVP calcula confirmados e gap por alvo, sem selecionar ou promover pessoas automaticamente; Staff atualiza por `PUT /events/:id/composition-targets`.
- Reserva generica usa `EventReserveEntry`, ordem, motivo Staff-only e estados `RESERVE`, `PROMOTION_PENDING`, `PROMOTED`, `DECLINED`, `REMOVED`. Staff inclui/remove/oferece vaga em `/events/:id/reserves/:playerId`; o player responde em `PUT /events/:id/reserve-response`. Aceite preserva o registro, audita e confirma somente o RSVP; nao marca presenca nem concede DKP. O player ve posicao/status, nunca o motivo interno. Conflitos entre confirmados sao calculados por sobreposicao e exibidos no timezone valido do player, com fallback `America/Sao_Paulo`.
- Lembretes nao criticos de evento usam `Player.eventReminderChannel` (`WEB`, `DISCORD`, `BOTH`, `NONE`) e podem ser alterados em `/dashboard/profile` por `PATCH /players/me/preferences`; o default conservador e Web. `sendUpcomingEventReminders` roda a cada hora, considera apenas eventos abertos nas proximas 24h e envia uma vez por evento/player somente quando o RSVP esta sem resposta ou `CONFIRMED`. `TENTATIVE`, `DECLINED`, ausencia no periodo e reserva ainda nao promovida nao recebem esse lembrete. `EventReminderDelivery` persiste idempotencia e retry de DM. A preferencia nao desliga alertas criticos nem as cobrancas obrigatorias de perfil/Codex.
- Ao finalizar um evento, RSVPs `CONFIRMED` sem presenca real e sem periodo de ausencia cobrindo o horario recebem `noShowDetectedAt`. O player consulta `GET /events/no-shows/me` e justifica por `PUT /events/:id/no-show-justification`; a Staff ve o contexto no resumo RSVP. Justificativa e Staff-only e auditada. Um no-show isolado nao altera DKP, presenca, elegibilidade, disciplina ou `riskFlags` automaticamente.
- Politicas publicadas usam `GuildPolicyVersion`, separado de `BusinessRule`. A Staff cria rascunho bilingue em `/dashboard/staff/rules`, com titulo/resumo PT-BR e EN, vigencia e snapshot apenas das regras publicaveis; pode atualizar o snapshot antes de publicar. `POST /guild-policies/drafts/:policyId/publish` atribui versao sequencial, autoria, horario e diff simples PT-BR/EN dentro de transacao serializavel. Depois de publicada, a versao e imutavel pela API. Alterar `BusinessRule` continua mudando a operacao, mas nao reescreve a politica vigente; a tela Staff mostra o drift entre ambas. Mudanca emergencial exige selo e motivo. A publicacao cria `GuildPolicyReceipt` para cada player ativo e notificacao bilingue idempotente; falhas isoladas de notificacao entram na auditoria sem reverter a versao. Players consultam versao vigente, proximas versoes, historico, diff e snapshot em `/dashboard/rules` por `GET /guild-policies`, registram abertura e `Li e entendi` por endpoints idempotentes e recebem a pendencia no action plan ate confirmar. O recibo registra apenas ciencia da informacao, nunca concordancia juridica ampla. A Staff ve cobertura agregada e nomes de quem ainda nao abriu quando operacionalmente necessario, sem qualquer dado de leilao sigiloso.
- Casos privados usam `GuildCase` e `GuildCaseEntry`. Players abrem duvida, denuncia operacional ou recurso em `/dashboard/cases`, acompanham apenas entradas `PLAYER` e podem reabrir um caso resolvido ao enviar novo contexto. A Staff opera `/dashboard/staff/cases` com categoria, severidade, responsavel Staff valido, prazo, status, historico e respostas obrigatoriamente separadas em PT-BR/EN; `INTERNAL_NOTE` nunca sai da visibilidade Staff. Resposta gera notificacao privada ao player. Toda criacao, triagem, mensagem, reabertura e resposta relevante e auditada, com `automaticDecision: false`; volume/severidade nunca pune, expulsa ou decide disciplina automaticamente. Contestacoes de leilao permanecem em `AuctionDispute` e sao apenas agregadas/referenciadas na central, sem duplicar status, review ou regras de sigilo.
...
- O mural player-facing de resultados entregues fica em `/dashboard/drops` e consome `GET /drops/auction-results`. Um resultado so e publicado depois que a Staff conclui a entrega com prova, criando `DropHistory` com leilao, vencedor, `deliveredAt` e `proofImageUrl`. O card mostra item, vencedor, horario e prova em PT-BR/EN, sem ranking, bids, locks, concorrentes ou custo. O mesmo gatilho publica o embed bilingue no webhook logico `drops`, com prova e link para o mural; a fila persistente permite retry Staff sem expor a URL.
...
- O ciclo de membresia do roster usa `Player.isActive`, `deactivatedAt`, `deactivationReason` e `reactivationRequestedAt`. A Staff desativa/reativa em `PATCH /players/:playerId/membership`, com motivo e auditoria; inativos ficam fora da chamada, dos recalculos de presenca e da matriz ativa. Se um inativo autenticar novamente, o retorno fica pendente no painel `/dashboard/staff/players`, o player ve `/access-review` em PT-BR/EN e todas as demais rotas autenticadas permanecem bloqueadas ate a liberacao Staff. A Staff nao pode desativar o proprio player.
...
- O dominio Staff-only `diamond-sales` controla vendas de itens habilitados por `ItemCatalog.diamondSaleEnabled`. `POST /diamond-sales` exige comprador interno vinculado ou nome externo, custodiante dos diamantes, total inteiro, print do item, prova da venda e modo `ALL_ACTIVE`/`EXCLUDE_SELECTED`. A abertura transacional grava `DiamondSale` e um snapshot imutavel em `DiamondSaleRecipient` apenas dos players com `isActive=true`; comprador interno e exclusoes manuais ficam fora. `shareAmount = floor(diamondTotal / recipientCount)` e `remainderAmount` preserva a sobra.
- A tela `/dashboard/staff/diamond-sales` calcula a previa antes da confirmacao, lista vendas e exige prova individual em `POST /diamond-sales/:saleId/recipients/:recipientId/deliver`. O ultimo envio conclui a venda automaticamente e publica uma entrega PT-BR/EN no alvo logico `drops`, com nomes, valores e todas as provas; falha deixa `discordPublishedAt` vazio e permite `POST /diamond-sales/:saleId/publish`. Mutacoes desse dominio respeitam modo manutencao, papeis Staff/Admin e auditoria.
- O catálogo de itens (`/dashboard/admin/items`, `ItemCatalog`) suporta importação em lote e leitura óptica de prints do jogo (OCR via `tesseract.js` no client-side) ou listas em texto. `POST /items/validate-batch` checa duplicatas em tempo real contra o banco de dados. `POST /items/bulk` insere múltiplos itens em lote dentro de transação, ignorando duplicatas e registrando `ITEM_CATALOG_BULK_CREATED` na auditoria. Suporta itens de grau `common` (branco) e `uncommon` (verde) sem obrigatoriedade de `itemTier`, atendendo à progressão inicial do Servidor Zero.
...
- A conversao de recrutamento cria `Player`, perfil de combate inicial, nota Staff e `PlayerOnboardingPlan` dentro da mesma transacao, vinculando `convertedPlayerId/convertedAt` na candidatura e bloqueando duplicidade por nick/usuario. O fluxo exige `userId` existente e nao automatiza Discord, roles ou permissoes externas. O plano nasce do `OnboardingTemplate` ativo; sem template, a primeira conversao cria o padrao G3X. Cada plano copia um snapshot bilingue das etapas para que uma nova versao nao reescreva o onboarding em andamento.
- Onboarding real usa `/dashboard/onboarding` e `GET /onboarding/me` para mostrar prazo, progresso obrigatorio/total e proximo passo PT-BR/EN. Etapas de regras, perfil, timezone, build, wishlist e primeiro evento usam evidencias existentes; presenca/DKP e canais usam confirmacao manual auditada por `PATCH /onboarding/me/steps/:stepId/complete`. A Staff usa `/dashboard/staff/onboarding`, `GET /onboarding/staff` e `POST /onboarding/staff/templates` para publicar templates versionados com etapas obrigatorias/opcionais. Conclusao e prazo sao acompanhamento, nunca punicao automatica.
- Trial transparente usa `/dashboard/trial` e `GET /player-trials/me`; o player recebe periodo, objetivo, criterios PT-BR/EN e somente check-ins D7/D14/D30 concluidos. Notas internas e motivos de ausencia marcados Staff-only sao filtrados. A Staff usa `/dashboard/staff/trials`, `GET/POST /player-trials/staff`, `POST /player-trials/staff/:trialId/check-ins/:day` e `POST /player-trials/staff/:trialId/decision`. Criterios sao snapshot imutavel publicado antes da avaliacao; ausencia declarada pausa o estado efetivo e estende o fim exibido sem cron mutavel. Aprovar, estender ou encerrar exige motivo bilingue e auditoria. O dominio nao cria score, nao decide automaticamente e nao altera loot, ranking, bids, locks ou sigilo.
- Mentoria usa `/dashboard/mentorship` e `/dashboard/staff/mentorship` sobre `/mentorship`. `MentorProfile` exige consentimento e disponibilidade voluntarios, com topicos/roles; Staff associa somente voluntario elegivel ou grupo de acolhimento. `MentorshipHelpRequest` registra pedido por conteudo/role sem exigir DM. Marcos de primeiro evento, boss, request, interesse e War Room sao datas calculadas dos dominios reais e nao pontos. Respostas de mentor selecionam somente identidade publica, associacoes e pedidos encaminhados: nao ha poder disciplinar nem acesso a `PlayerStaffNote`.
- Pulso voluntario usa `/dashboard/pulse` e `/dashboard/staff/pulse` sobre `/guild-pulse`. `GuildPulseResponse` guarda cinco notas sem `playerId`; `GuildPulseParticipation` guarda apenas recibo `SUBMITTED/SKIPPED` separado e sem chave para resposta. API Staff bloqueia medias e textos abaixo de `minGroupSize` (padrao 5, minimo 3) e nunca retorna score individual. Texto aberto opcional nasce pendente, exige moderacao, expira em 1-90 dias e e removido diariamente pelo cron `cleanupExpiredPulseText`; nota anonima permanece para tendencia agregada. Envio/skip nao geram audit com ator; configuracao/status/moderacao geram. Pular nao possui consequencia e pulso nao afeta loot, DKP, acesso ou disciplina. Politica: `docs/GUILD_PULSE_DATA_POLICY.md`.
- Saude explicavel usa `GET /guild-health/signals` e `/dashboard/staff/guild-health`. Queda de participacao compara ultimos 15 dias com 15 anteriores somente com pelo menos dois eventos em cada janela; onboarding parado usa 14 dias/prazo; confirmacoes revertidas sao transicoes `CONFIRMED` para outro status reconstruidas do audit; retorno usa `PLAYER_REACTIVATED` em 30 dias; coorte isolada agrega classe com pelo menos tres ativos e menos de 25% com presenca em 30 dias. Cada card expoe fatos, janela e recomendacao de conversa. Nao existe score unico, mutacao, bloqueio, remocao ou efeito em loot.
- Saude da lideranca usa `/leadership-health` e `/dashboard/staff/leadership-health`. Cada Staff registra carga 1-5, disponibilidade para plantao e contexto por `LeadershipArea` (eventos, loot, recrutamento, Discord, deploy, tesouraria e cuidado com players). Painel usa ultimo check-in por pessoa/area e acoes auditadas dos ultimos 14 dias para mostrar cobertura sem substituto e concentracao >=60% com amostra minima de cinco. Alertas recomendam delegar/reduzir/pausar; nao alteram permissao, nao escalam automaticamente e nao pedem mais atividade.
- Fila Staff usa `/staff-tasks` e `/dashboard/staff/tasks`. `StaffTask` persiste area, prioridade, status, dono, substituto, prazo, deep link e fonte; `sourceKey` unico evita duplicar conversao. Briefing matinal, pauta nao resolvida e sinais de saude sao sugestoes read-only e so viram tarefa por `POST` confirmado (`automaticTaskCreation=false`). `StaffTaskHandoff` exige contexto final/proximo passo e pode transferir dono em transacao; criacao, mudanca e handoff sao auditados. `/dashboard/staff/roadmap` apresenta todas as frentes da release 2026-07-21, seus links de evidencia e a coleta operacional ainda pendente. O plano de campo aponta o proximo perfil de entrevista, oferece roteiro canonico copiavel e organiza quatro semanas oficiais de 27/07 a 24/08, liberando o atalho de captura somente depois do encerramento. O mesmo painel consome `/product-validation`: entrevistas anonimizadas registram perfil/canais/visibilidade de ausencia/sintese sem identidade ou conteudo privado; semanas encerradas congelam metricas automaticas de evento, presenca, no-show, recruit ativo e tarefa sem substituto, separadas de presenca esperada/minutos de cobranca declarados. O gate exige tres perfis Staff, ao menos cinco players cobrindo veterano/novato/ativo/baixa atividade, quatro semanas consecutivas iniciadas na segunda-feira em `America/Sao_Paulo` e evidencia de reducao de cobranca por RSVP; todas as escritas sao Staff-only e auditadas.
- Cobertura Staff usa `/staff-coverage` e `/dashboard/staff/coverage`. `StaffAreaCoverage` separa primario, backup, janela local e timezone por `LeadershipArea`; `StaffAvailabilityPeriod` guarda indisponibilidade declarada pelo proprio membro. O responsavel efetivo muda para o backup somente durante declaracao vigente, nunca por silencio. Configuracao, declaracao e remocao sao auditadas e nao alteram roles ou permissoes.
- Automacao Staff usa `/staff-automations` e `/dashboard/staff/automations`. Somente agrupamentos de `StaffTask` com mesma origem/area e ao menos tres conclusoes em 90 dias viram proposta. Dry-run cria `StaffAutomationRule` desabilitada; ativacao exige confirmacao separada. O executor so cria `StaffTask` sem dono, respeitando frequencia, limite diario e `StaffAutomationRun.dedupKey`; kill switch desliga a regra e toda etapa e auditada. Loot, remocao de player e politica social nao possuem acao executavel nesse dominio.
- Playbooks usam `/playbooks`, `/dashboard/staff/playbooks` e `/dashboard/playbook`. `GuildPlaybookVersion` e imutavel e pode ser anexada exatamente a um `Event` ou `WarRoomOperation`. Brief/objectivo sao PT-BR/EN; notas Staff ficam separadas; `PlaybookRoleInstruction` entrega ao player escalado somente o proprio papel. `PlaybookInstructionReceipt` confirma leitura sem alterar RSVP, presenca ou DKP. Sinais do after-action viram candidatos; `GuildPlaybookLesson` exige `KEEP/TEST/DISCARD`, dono e revisao. Promover `KEEP/TEST` cria nova versao com origem na operacao/licao.
- Comunicacao adaptativa usa `/communications` e `/dashboard/communications`. `PlayerCommunicationPreference` define Web/Discord/ambos/nenhum por eventos, loot proprio, requests, progresso, comunicados e lembretes, mais timezone, quiet hours, cadencia e bypass critico explicito. Digest agrupa apenas notificacoes do proprio user/player por tipo+link, preserva prazo e usa `PlayerDigestDelivery` para idempotencia diaria/semanal. O bot registra `/erp-rsvp`, `/erp-ausencia`, `/erp-instrucao` e `/erp-regra`, resolve `discordId` e chama os mesmos servicos de dominio da Web; site permanece fonte de verdade.
- O smoke autenticado grava resultado sanitizado, duracao total e latencia por check em `SMOKE_RESULT_PATH`; Actions preserva o artefato por 30 dias e verifica tambem fila/cobertura/automacao Staff, playbooks, comunicacao e telemetria agregada. Incidentes de webhook seguem `docs/WEBHOOK_INCIDENT_RUNBOOK.md`; retencao social segue `docs/DATA_RETENTION_POLICY.md`. Esses documentos nao autorizam expurgo automatico de caso sob revisao.
- `GET /operations/staff/product-telemetry?days=7..90` entrega somente agregados de recrutamento, onboarding, RSVP, tarefas Staff, leiloes, entregas, requests e interesses, incluindo tempos medios operacionais. A resposta declara e testa ausencia de identidade de player e texto privado; nao cria score individual nem efeito em loot, trial ou disciplina.
...
- A nova rodada de produto fica em `docs/RAVEN2_GUILD_OPERATING_SYSTEM_ROADMAP_2026_07.md`. Ela reconcilia os roadmaps anteriores com dores publicas de guildas de MMORPGs e prioriza compromissos/RSVP/ausencias, governanca versionada, onboarding/mentoria, continuidade da Staff, playbooks e comunicacao adaptativa. O prelude e as Frentes 1/2/3/4/5/6/7/9 integram a release 2026-07-21. A Frente 8 permanece futura ate existir caso real validado; a Frente 0 aguarda entrevistas e quatro semanas de metricas reais G3X.
...
- A mesma tela mostra prontidao do boss via `GET /events/:id/readiness`,
  ...
  progresso `STATUS`. STATUS ausente ou com mais de 21 dias entra como
...
- `scripts/authenticated-smoke-test.js` roda o smoke autenticado pos-deploy com Bearer token de automacao (`SMOKE_AUTH_TOKEN` ou `SMOKE_BEARER_TOKEN`), validando `/auth/me`, central Staff, diagnostico de leilao, entregas pendentes, setup Staff de vendas por diamantes, health privado e painel de deploy. No GitHub Actions, o job `deploy-smoke` executa a etapa se o secret `PRODUCTION_SMOKE_BEARER_TOKEN` estiver configurado; sem o secret, a etapa e pulada explicitamente sem falhar o deploy.
...
- `docs/RAVEN2_GUILD_OPERATING_SYSTEM_ROADMAP_2026_07.md`: nova rodada de descoberta e roadmap para compromissos, saude/retencao, governanca, onboarding, continuidade da Staff, conhecimento e comunicacao.
- `docs/RAVEN2_GUILD_OPERATING_SYSTEM_COMPLETION_AUDIT_2026_07.md`: matriz de conclusao do objetivo, evidencias por fatia e pendencias externas que nao podem ser simuladas.
...
| 2026-07-21 | CI ganhou PostgreSQL descartavel com todas as migrations e E2E HTTP dos quatro fluxos transacionais criticos; autoria de evento tambem ficou presa a sessao. | CI/E2E/seguranca/roadmap |
| 2026-07-21 | Controllers mutaveis legados ganharam validacao estrita/testes, autoria passou a vir apenas da sessao e a Staff ganhou telemetria agregada sem identidade ou texto privado. | seguranca/confiabilidade/roadmap |
| 2026-07-21 | Smoke autenticado passou a preservar artefato sanitizado/duracao e cobrir novas jornadas; runbook de webhook e politica de retencao foram formalizados. | confiabilidade/roadmap |
| 2026-07-21 | Comunicacao adaptativa ganhou canal por categoria, quiet hours, digest idempotente sem terceiros e comandos Discord que reutilizam os contratos canonicos da Web. | comunicacao/Discord/roadmap |
| 2026-07-21 | Playbooks ganharam versoes imutaveis anexadas a evento/War Room, licoes decididas do after-action e leitura do proprio papel separada de RSVP, sem notas Staff. | playbook/War Room/roadmap |
| 2026-07-21 | Automacao Staff segura passou a propor apenas padroes repetidos, exigir dry-run/confirmacao e criar somente tarefa interna com limites, idempotencia, auditoria e kill switch. | Staff/automacao/roadmap |
| 2026-07-21 | Staff ganhou cobertura por area com primario, backup, janela/timezone e indisponibilidade declarada; escalonamento nunca usa silencio e nao altera permissoes. | Staff/continuidade |
| 2026-07-21 | Fatia 3.1 substituiu checklist decorativo por template Staff versionado e plano persistido por recruit, com snapshot, prazo, proximo passo bilingue e conclusao verificavel. | onboarding/recrutamento/roadmap |
| 2026-07-21 | Fatia 3.2 criou trial transparente com criterios bilingues imutaveis, check-ins D7/D14/D30, pausa por ausencia e decisoes humanas justificadas/auditadas sem efeito em loot. | trial/onboarding/roadmap |
| 2026-07-21 | Fatia 3.3 criou mentoria voluntaria/grupo de acolhimento, pedidos de ajuda por conteudo/role e marcos factuais sem pontos, disciplina ou notas Staff. | mentoria/onboarding/roadmap |
| 2026-07-21 | Fatia 4.1 criou pulso opcional com respostas sem identidade, agregado por grupo minimo, texto moderado/expiravel e politica de dados explicita. | saude/privacidade/roadmap |
| 2026-07-21 | Fatia 4.2 criou sinais Staff explicaveis por fatos/janelas para conversa, sem score de lealdade, mutacao automatica ou efeito em loot. | saude/sinais/roadmap |
| 2026-07-21 | Fatia 4.3 criou check-in Staff de carga/plantao e painel de cobertura/concentracao que recomenda delegar ou pausar sem alterar permissao. | lideranca/saude/roadmap |
| 2026-07-21 | Fatia 5.1 criou fila Staff persistente com sugestoes confirmaveis, dono, substituto, prazo, deep link, handoff e pagina bonita do roadmap para validacao. | Staff/tarefas/roadmap |
| 2026-07-21 | Fatia 2.3 criou casos privados com conversa player/Staff, nota interna separada, dono, prazo e historico, referenciando contestacoes de leilao sem duplicar o dominio e sem decisao disciplinar automatica. | governanca/casos/roadmap |
| 2026-07-21 | Fatia 2.2 adicionou recibo idempotente de abertura/ciencia, action plan e inbox bilingue, cobertura Staff e mudanca emergencial com motivo, sem transformar ciencia em concordancia juridica. | governanca/politicas/roadmap |
| 2026-07-21 | Fatia 2.1 separou regras operacionais de politicas publicadas imutaveis, com rascunho, versao, vigencia, autoria, diff bilingue e drift visivel. | governanca/politicas/roadmap |
| 2026-07-21 | Fatia 1.4 concluiu lembretes de evento por canal, limitados a sem resposta/confirmados, e no-show justificavel sem punicao ou risk flag automatica. | eventos/lembretes/no-show/roadmap |
| 2026-07-21 | Fatia 1.3 implementou series recorrentes com pausa/excecoes, alvos de composicao sem selecao automatica, reserva confirmada pelo player e conflitos no timezone individual. | eventos/recorrencia/reserva/roadmap |
| 2026-07-21 | Fatia 1.2 implementou periodos de ausencia privados por padrao, cobertura automatica de compromissos e impacto agregado na composicao Staff, sem notificacao invasiva. | eventos/ausencias/roadmap |
| 2026-07-21 | Fatia 1.1 do Guild Operating System implementou RSVP por evento com vou/talvez/nao vou, nota privada por padrao, composicao Staff e separacao total de presenca/DKP. | eventos/RSVP/roadmap |
| 2026-07-21 | Players ativos passaram a receber uma cobranca privada diaria e consolidada por perfil incompleto, STATUS vencido em 21d, presenca abaixo de 50% em 15d e Codex enviado sem resposta; o envio possui idempotencia persistida e DM imediata para Codex. | players/automacao/Discord/Codex |
| 2026-07-20 | Criado roadmap Guild Operating System apos auditoria do ERP e pesquisa de dores de guildas em MMORPGs; RSVP/ausencias foi priorizado como primeira hipotese, sem iniciar implementacao. | produto/roadmap |
| 2026-07-20 | Staff ganhou vendas por diamantes com itens habilitados, comprador interno/externo, snapshot de ativos, exclusoes, divisao inteira com sobra, provas da venda e de cada envio, conclusao e publicacao bilingue no Discord. | diamond-sales/Staff/Discord |
...
| 2026-07-20 | Protocolo Codex passou a exigir atualizacao ou novo passo nas Centrais Discord para toda implementacao que altere fluxo visivel, com dry-run antes do commit e publicacao depois da verificacao em producao. | AGENTS/Discord/guias |
| 2026-07-20 | Discord ganhou a Central da Staff exclusiva com 18 tutoriais PT-BR, imagens proprias, tags, indice, permissoes restritas e publicador idempotente versionado. | Discord/guias Staff |
| 2026-07-20 | Discord ganhou a Central do Player com 13 tutoriais bilingues, imagens proprias, tags, links diretos e threads abertas para duvidas; publicador idempotente ficou versionado para manutencao. | Discord/guias player |
