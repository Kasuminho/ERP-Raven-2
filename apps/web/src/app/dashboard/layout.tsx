    ...
    <AuthGuard>
      ...
      <div className="min-h-screen">
        ...
        <aside className="fixed inset-y-0 left-0 z-20 hidden w-72 border-r border-white/10 bg-background/88 p-4 shadow-[18px_0_60px_rgba(0,0,0,0.35)] backdrop-blur-xl lg:block">
          ...
          <nav className="max-h-[calc(100vh-9rem)] space-y-4 overflow-y-auto pr-1 scrollbar-thin" aria-label="Navegacao principal">
            {navGroups.map((group) => (
              <div key={group.label}>
                <p className="mb-1 px-3 text-[10px] font-bold uppercase tracking-[0.16em] text-muted-foreground/75">{group.label}</p>
                <div className="space-y-1">
                  {nav.filter((item) => group.hrefs.includes(item.href)).map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className={cn(
          ...
          <header className="sticky top-0 z-10 border-b border-white/10 bg-background/78 px-4 py-3 backdrop-blur lg:hidden">
            <div className="flex items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button variant="ghost" className="h-10 w-10 px-0" onClick={() => setMobileMenuOpen(true)} aria-label="Abrir menu"><Menu className="h-5 w-5" /></Button>
              </div>
            </div>
          </header>
        ...
        <nav className="fixed bottom-0 left-0 right-0 z-30 flex snap-x gap-1 overflow-x-auto border-t border-white/10 bg-background/94 px-2 py-2 shadow-[0_-18px_55px_rgba(0,0,0,0.35)] backdrop-blur-xl lg:hidden">
          ...
          <button type="button" onClick={() => setMobileMenuOpen(true)} className="flex min-w-16 snap-start flex-col items-center gap-1 rounded-md border border-transparent px-2 py-1.5 text-[10px] text-muted-foreground transition hover:bg-muted" aria-label="Mais destinos">
            <Menu className="h-4 w-4" /><span>Mais</span>
          </button>
        </nav>
        {mobileMenuOpen && (
          <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm lg:hidden" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && setMobileMenuOpen(false)}>
            <section className="absolute inset-x-0 bottom-0 max-h-[88vh] overflow-y-auto rounded-t-2xl border-t border-primary/25 bg-card p-4 shadow-2xl" role="dialog" aria-modal="true" aria-labelledby="mobile-menu-title">
              <div className="mb-4 flex items-center justify-between">
                <div><h2 id="mobile-menu-title" className="font-[var(--font-cinzel)] text-xl font-bold">Raven Command</h2><p className="text-xs text-muted-foreground">Tudo no lugar, sem maratona de icones.</p></div>
                <Button variant="ghost" className="h-10 w-10 px-0" onClick={() => setMobileMenuOpen(false)} aria-label="Fechar menu"><X className="h-5 w-5" /></Button>
              </div>
              <GlobalSearch />
              <div className="mt-5 space-y-5">
                {navGroups.map((group) => (
                  <div key={group.label}>
                    <p className="mb-2 text-xs font-bold uppercase tracking-[0.14em] text-primary">{group.label}</p>
                    <div className="grid grid-cols-2 gap-2">
                      {nav.filter((item) => group.hrefs.includes(item.href)).map((item) => (
                        <Link key={item.href} href={item.href} onClick={() => setMobileMenuOpen(false)} className="flex min-h-12 items-center gap-2 rounded-lg border border-white/10 bg-background/45 p-3 text-sm hover:border-primary/35">
                          <item.icon className="h-4 w-4 shrink-0 text-primary" /><span className="truncate">{t(locale, item.label)}</span>
                        </Link>
                      ))}
                    </div>
                  </div>
                ))}
                {isStaff && <Link href="/dashboard/staff" onClick={() => setMobileMenuOpen(false)} className="flex min-h-12 items-center gap-2 rounded-lg border border-primary/30 bg-primary/10 p-3 font-semibold text-primary"><ShieldAlert className="h-4 w-4" />{t(locale, 'staff')}</Link>}
                <Button variant="ghost" className="w-full" onClick={logout}><LogOut className="h-4 w-4" />{t(locale, 'signOut')}</Button>
              </div>
            </section>
          </div>
        )}
